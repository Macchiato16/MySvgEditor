    ...
    if(seclectedShapeList.size() > 0)
        {
            ...
            copyFlag = true;
            copyPoint = prePoint;  // 记录复制点
        }
    else if(seclectedShape){
        copyShapeList.clear();
        copyShapeList << seclectedShape;
        copyFlag = true;
        copyPoint = prePoint;
    }
}
...
void CanCvns::onPasteshapeTriggered()
{
    ...
    if (!copyShapeList.isEmpty())
    {
        ...
        {
            cutFlag = false;
            copyShapeList.clear();  // 清除剪切板中的内容
        }
    }
    update();
    resetMouseMenu();  // 更新右键菜单状态
}
...
void CanCvns::onCutshapeTriggered()
{
    //saveStateForUndo();  // 保存当前状态
    if(seclectedShapeList.size() > 0)
    {
        ...
        cutFlag = true;
        onDeleteshapeTriggered();  // 删除选中的形状
    }
...
//置上一层事件
void CanCvns::onBringForwardTriggered(){

}
//置下一层事件
void CanCvns::onSendBackwardTriggered(){

}
void CanCvns::onOverwriteShapeTriggered()
{
    this->onCopyshapeTriggered();
    saveStateForUndo();  // 保存当前状态
    if (!copyShapeList.isEmpty())
    {
        pastePoint = prePoint+QPoint(20,20);  // 粘贴点偏移量
        for (auto tmpshape : copyShapeList)
        {
            pasteCopedShape(tmpshape);
        }
...
void CanCvns::mousePressEvent(QMouseEvent *event)
{
    ...
    if(currShape != MyShapeType::noshape && Qt::LeftButton == event->button())
    {
        ...
        case MyShapeType::star: shape = new MyStar(); break;  // 新增五角星类型
        default:
        {
            shape = nullptr;
            break;
...
void CanCvns::setMouseMenu()
{
    rightClick = new QMenu(this);
        selectAllAction = new QAction(rightClick);
        copyShape = new QAction(rightClick);
        cutShape = new QAction(rightClick);
        pasteShape = new QAction(rightClick);
        overwriteShape = new QAction(rightClick);
        ...
        undoAction = new QAction(rightClick);
        redoAction = new QAction(rightClick);
        bringForward = new QAction(rightClick);
        sendBackward = new QAction(rightClick);


        selectAllAction->setText(tr("全选"));
        cutShape->setText(tr("剪切"));
        copyShape->setText(tr("复制"));
        pasteShape->setText(tr("粘贴"));
        overwriteShape->setText(tr("复写"));
        deleteShape->setText(tr("删除"));
        bringForward->setText(tr("置上一层"));
        sendBackward->setText(tr("置下一层"));
        ...
        rightClick->addAction(selectAllAction);
        rightClick->addAction(cutShape);
        rightClick->addAction(copyShape);
        rightClick->addAction(pasteShape);
        rightClick->addAction(overwriteShape);
        rightClick->addAction(deleteShape);
        rightClick->addAction(bringForward);
        rightClick->addAction(sendBackward);
        rightClick->addAction(toUp);
        rightClick->addAction(toDown);
        rightClick->addAction(secondEdit);
        rightClick->addSeparator();  // 添加一个分隔符
        rightClick->addAction(undoAction);
        rightClick->addAction(redoAction);

        connect(selectAllAction, SIGNAL(triggered()), this, SLOT(selectAllShapes()));
        connect(cutShape, SIGNAL(triggered()), this, SLOT(onCutshapeTriggered()));
        ...
        connect(bringForward, &QAction::triggered, this, [&] { upORdown = true; swapShape(); });
        connect(sendBackward, &QAction::triggered, this, [&] { upORdown = false; swapShape(); });
        ...
        connect(overwriteShape,SIGNAL(triggered()), this, SLOT(onOverwriteShapeTriggered()));
}
...
void CanCvns::resetMouseMenu()
{
    ...
    overwriteShape->setEnabled(false);
    ...
    if (nullptr == seclectedShape)
    {
        ...
        if (!seclectedShapeList.isEmpty())
        {
            copyShape->setEnabled(true);
            overwriteShape->setEnabled(true);
...
bool CanCvns::KosoSvgHander(const QString &filename)
{
    ...
    while (false == reader.atEnd())
    {
        ...
        if (reader.name() == "g")
        {
            ...
            else if (true == starFlag)  // 读取五角星
            {
                shape = new MyStar();
                shapetype = MyShapeType::star;
                shape->setPos(x, y, x + width, y + height);
                shape->setFillColor(transQColor(fillColor));
            }
            else
        ...
        else if (reader.name() == "star")  // 识别五角星
                {
                    starFlag = true;
                    x = reader.attributes().value("x").toInt();
                    y = reader.attributes().value("y").toInt();
...
void CanCvns::pasteCopedShape( MyShape *copedShape)
{
    ...
    case MyShapeType::star:
            shape = new MyStar(*static_cast<MyStar*>(copedShape));
            break;
    default:
    {
        shape = nullptr;
...
void CanCvns::selectAllShapes()
{
    seclectedShapeList.clear();
    foreach(MyShape *shape, shapeList)
    {
        seclectedShapeList.append(shape);
    }
    update();
}
QList<MyShape*> CanCvns::deepCopyShapeList(const QList<MyShape*>& originalList)
{
    QList<MyShape*> copyList;
    for (auto shape : originalList)
    {
        MyShape* copiedShape = nullptr;
        switch (shape->getType())
        {
        case MyShapeType::freeline:
            copiedShape = new MyFreeLine(*static_cast<MyFreeLine*>(shape));
            break;
        case MyShapeType::line:
            copiedShape = new MyLine(*static_cast<MyLine*>(shape));
            break;
        case MyShapeType::rect:
            copiedShape = new MyRect(*static_cast<MyRect*>(shape));
            break;
        case MyShapeType::circle:
            copiedShape = new MyCircle(*static_cast<MyCircle*>(shape));
            break;
        case MyShapeType::pentagon:
            copiedShape = new MyPentagon(*static_cast<MyPentagon*>(shape));
            break;
        case MyShapeType::trangle:
            copiedShape = new MyTriangle(*static_cast<MyTriangle*>(shape));
            break;
        case MyShapeType::text:
            copiedShape = new MyText(*static_cast<MyText*>(shape));
            break;
        case MyShapeType::star:
            copiedShape = new MyStar(*static_cast<MyStar*>(shape));
            break;
        default:
            break;
        }
...
void CanCvns::undo()
{
    if (undoStack.isEmpty())
        ...
        redoStack.push(deepCopyShapeList(shapeList));  // 保存当前状态到重做堆栈
        shapeList = undoStack.pop();  // 恢复上一个状态
        update();
}
...
void CanCvns::redo()
{
    if (redoStack.isEmpty())
        ...
        undoStack.push(deepCopyShapeList(shapeList));  // 保存当前状态到撤销堆栈
        shapeList = redoStack.pop();  // 恢复下一个状态
        update();
}

void CanCvns::saveStateForUndo()
{
    undoStack.push(deepCopyShapeList(shapeList));  // 使用深拷贝
