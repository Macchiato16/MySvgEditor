...
public slots:
    ...
    //剪切对象事件
    void onCutshapeTriggered();
    //置上一层事件
    void onBringForwardTriggered();
    //置下一层事件
    void onSendBackwardTriggered();
    //复写对象事件
    void onOverwriteShapeTriggered();
    //画笔尺寸
...
private:
    ...
    QAction *pasteShape;
    QAction *deleteShape;
    QAction *overwriteShape;
    QAction *toUp;
    QAction *toDown;
    QAction *secondEdit;
    QAction *undoAction;
    QAction *redoAction;
    QAction *selectAllAction;  // 全选操作
    QAction *cutShape;  // 新增剪切操作
    QAction *bringForward;
    QAction *sendBackward;

    // 新增撤销和重做的堆栈
    QStack<QList<MyShape*>> undoStack;
    QStack<QList<MyShape*>> redoStack;
    ...
    void breakEdit();
    void saveStateForUndo(); // 新增保存状态的函数
    QList<MyShape*> deepCopyShapeList(const QList<MyShape*>& originalList);  // 深拷贝函数声明
